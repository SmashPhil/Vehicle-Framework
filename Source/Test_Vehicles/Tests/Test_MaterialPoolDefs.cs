...
internal sealed class Test_MaterialPoolDefs : Test_VehicleDefTest
{
  protected override bool ShouldTest(VehicleDef vehicleDef)
  {
    const string HotReloadSuffix = "_HotReloadedThrowaway";
    ...
    return !vehicleDef.defName.EndsWith(HotReloadSuffix);
  }
  ...
  private void Total()
  {
    ...
    Expect.AreEqual(RGBMaterialPool.TotalMaterials, count);
  }
}