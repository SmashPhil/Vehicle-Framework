...
internal sealed class Test_VehiclePawn_Pathing
{
  ...
  private void OrderGoto()
  {
    ...
    FloatMenuOptionProvider_OrderVehicle.PawnGotoAction(gotoLoc, group.vehicle, gotoLoc,
      Rot8.North);
    Assert.AreEqual(JobDefOf.Goto, group.vehicle.jobs.curJob.def);
  }
  ...
  private void OrderCorridorEvenWidth()
  {
    ...
    using DestroyAreaScope das = new(group.vehicle.Map, testArea);
  ...
  public readonly struct DestroyAreaScope(Map map, CellRect rect) : IDisposable
  {
    ...
    {
      DebugHelper.DestroyArea(rect, map);
    }
  }
}