...
// TODO - Testing layered graphics, right now only parent and child primary graphics are checked.

[UnitTest(TestType.Playing)]
[TestCategory(TestCategoryNames.ParallelRenderer, TestCategoryNames.VehicleTurret)]
[TestDescription("VehicleTurret rendering through the parallel renderer system.")]
internal sealed class UnitTest_VehicleTurret_Rendering
{
  ...
  private void AlignAllRotations()
  {
    ...
    rootTurret = vehicle.CompVehicleTurrets.CopyAndAddTurret(new VehicleTurret(vehicle)
    {
    ...
    childTurret = vehicle.CompVehicleTurrets.CopyAndAddTurret(new VehicleTurret(vehicle)
    {
      def = new VehicleTurretDef
      {
        defName = "TurretDef2",
