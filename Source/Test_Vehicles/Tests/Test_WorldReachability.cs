...
internal sealed class Test_WorldReachability
{
  [Test]
  private void RegionFloodFill()
  {
    ...
    {
      //using Test.Scope group = new(vehicleDef.defName);
  ...
  private void RegionGeneration()
  {
    ...
    foreach (VehicleDef vehicleDef in GridOwners.World.AllOwners)
    {
      ...
      foreach ((int id, List<int> tiles) in regions)
      {
        ...
        Expect.All(tiles, tile => regionGrid.GetRegionId(tile) == id);
        ...
        if (id == -1)
        {
          ...
          Expect.AreEqual(expectedTiles.Count, 0);
        }
        else
        {
          Expect.All(expectedTiles, tile => regionGrid.GetRegionId(tile) == id);
          Expect.AreEqual(expectedTiles.Count, tiles.Count);
        }
        totalCount += tiles.Count;
      }
      Expect.AreEqual(totalCount, Find.WorldGrid.TilesCount);
