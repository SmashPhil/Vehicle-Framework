...
internal sealed class Test_MapGeneration_Coast
{
  ...
  private static readonly HarmonyMethod CoastOffsetPostfixMethod =
    new(AccessTools.Method(typeof(Test_MapGeneration_Coast), nameof(CoastOffsetAtPostfix)),
      HarmonyLib.Priority.First);

  private static readonly HarmonyMethod CoastOffsetFinalizerMethod =
    new(AccessTools.Method(typeof(Test_MapGeneration_Coast), nameof(CoastOffsetAtFinalizer)));
