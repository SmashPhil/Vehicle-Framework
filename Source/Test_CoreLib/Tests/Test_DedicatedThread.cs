...
internal class Test_DedicatedThread
{
  ...
  private void Dispatcher()
  {
    ...
    Expect.IsTrue(resetEvent.Wait(TimeSpan.FromMilliseconds(WaitTime)),
      "Wait handle expired before thread could unblock.");
    resetEvent.Reset();

    Assert.AreEqual(dedicatedThread.QueueCount, 0);
    Expect.IsTrue(dedicatedThread.IsBlocked);
  }
