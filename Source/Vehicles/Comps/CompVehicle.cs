    ...
    public class CompVehicle : ThingComp
    {
        ...
        public bool MovementHandlerAvailable
        {
            get
            {
                ...
                if (Pawn.GetCachedComp<CompRefuelable>() != null && Pawn.GetCachedComp<CompRefuelable>().Fuel <= 0f)
                    return false;
                return true;
            }
        }
        ...
        public IEnumerable<IntVec3> SurroundingCells
        {
            get
            {
                return Pawn.OccupiedRect().ExpandedBy(1).EdgeCells;
            }
        }

        public int TotalAllowedFor(JobDef jobDef)
        {
            if(Props.vehicleJobLimitations.AnyNullified(v => v.defName == jobDef.defName))
            {
                return Props.vehicleJobLimitations.Find(v => v.defName == jobDef.defName).maxWorkers;
            }
            return 1;
        }
        ...
        public void AddHandlers(List<VehicleHandler> handlerList)
        {
            ...
            {
                VehicleHandler existingHandler = Pawn.GetCachedComp<CompVehicle>().handlers.FirstOrDefault(h => h == handler);
        ...
        public void RemoveHandlers(List<VehicleHandler> handlerList)
        {
            ...
            foreach(VehicleHandler handler in handlerList)
            {
                VehicleHandler vehicleHandler = Pawn.GetCachedComp<CompVehicle>().handlers.FirstOrDefault(h => h == handler);
            }
        }
