...
namespace Vehicles
{
    public class CompCannons : VehicleComp
    {
        ...
        public void RemoveCannons(List<CannonHandler> cannonList)
        {
            if (cannonList.NullOrEmpty())
                return;
            foreach(CannonHandler cannon in cannonList)
        ...
        private void ResolveCannons()
        {
            ...
            if(multiFireCannon?.Count > 0)
            {
                for(int i = 0; i < multiFireCannon.Count; i++)
                {
                    ...
                    if(!multiFireCannon[i].Second.cannonTarget.IsValid || multiFireCannon[i].Second.shellCount <= 0)
                    {
                        multiFireCannon.RemoveAt(i);
                        return;
                    }

                    multiFireCannon[i].Second.AlignToTargetRestricted();
                    ...
                    if (PairedData.First <= 0)
                    {
                        ...
                        if (multiFireCannon[i].First == 0 || (multiFireCannon[i].Second.cannonDef.ammoAllowed.AnyNullified() && multiFireCannon[i].Second.shellCount <= 0))
                        {
        ...
        public void FireTurretCannon(CannonHandler cannon, ref SPTuple2<int,int> data)
        {
            ...
            ThingDef projectile;
            if(!cannon.cannonDef.ammoAllowed.NullOrEmpty() && !cannon.cannonDef.genericAmmo)
            {
