...
public class CompFueledTravel : VehicleComp, IRefundable
{
  ...
  public override bool TickByRequest => !Props.ElectricPowered || Mathf.Approximately(Props.dischargeRate, 0);
    ...
    foreach (OffsetMote offset in Props.motesGenerated)
    {
      for (int i = 0; i < offset.NumTimesSpawned; i++)
      {
        try
        {
          ...
          MoteGenerator.ThrowMote(motePosition.ToIntVec3(), parent.Map, mote);
        }
        catch (Exception ex)
        {
          Log.Error(
