...
public class WITab_AerialVehicle_Health : WITab_AerialVehicle
{
  ...
  private List<PawnCapacityDef> CapacitiesToDisplay
  {
    get
    {
      ...
      List<PawnCapacityDef> allDefsListForReading =
  ...
  private void DoColumnHeaders(ref float curY)
  {
    if (!compactMode)
    {
      ...
      {
        Widgets.Label(new Rect(num, 3f, PawnLabelColumnWidth, PawnLabelColumnWidth),
          list[i].LabelCap.Truncate(PawnLabelColumnWidth, null));
        num += PawnLabelColumnWidth;
      }
  ...
  private void DoRow(Rect rect, Pawn p)
  {
    ...
    Rect bgRect = new Rect(rect3.xMax + SpaceAroundIcon, 11f, PawnLabelColumnWidth,
      PawnLabelHeight);
    ...
    if (!compactMode)
    {
      ...
      for (int i = 0; i < list.Count; i++)
      {
        ...
        if ((p.RaceProps.Humanlike && !list[i].showOnHumanlikes) ||
          (p.RaceProps.Animal && !list[i].showOnAnimals) ||
          (p.RaceProps.IsMechanoid && !list[i].showOnMechanoids) ||
          !PawnCapacityUtility.BodyCanEverDoCapacity(p.RaceProps.body, list[i]))
        {
          num += PawnCapacityColumnWidth;
        }
        else
