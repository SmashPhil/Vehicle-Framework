...
public class ITab_Vehicle_Upgrades : ITab
{
  ...
  private VehiclePortrait portraitBefore;
  private VehiclePortrait portraitAfter;
  private bool closed;
  ...
  private UpgradeNode SelectedNode
  {
    ...
    set
    {
      if (selectedNode != value)
      {
        ...
        {
          portraitBefore.MarkDirty();
          portraitAfter.MarkDirty();
          RecacheTextEntries();
  ...
  public override void OnOpen()
  {
    ...
    portraitBefore = new VehiclePortrait();
    portraitAfter = new VehiclePortrait();
    closed = false;
  }
  ...
  protected override void CloseTab()
  {
    ...
    portraitBefore.Dispose();
    portraitAfter.Dispose();
    closed = true;
  }
  ...
  {
    // May occur if sub-window is open while selected entity changes inbetween frames, or if closing with
    // some keypress event since RimWorld's Window type can let through trailing event types on close.
    if (closed || Vehicle == null)
      return;
  ...
  private void DrawUpgradeNodes(Rect rect)
  {
    ...
    foreach (UpgradeNode upgradeNode in Vehicle.CompUpgradeTree.Props.def.nodes)
    {
      ...
      using WidgetGroupScope group = new(upgradeRect);
      upgradeRect = upgradeRect.AtZero();

      bool colored = false;
      bool disabled = Vehicle.CompUpgradeTree.Disabled(upgradeNode);
      bool prereqsMet = Vehicle.CompUpgradeTree.PrerequisitesMet(upgradeNode);
      if (disabled || !prereqsMet)
      {
        colored = true;
      ...
      if (Mouse.IsOver(upgradeRect))
      {
        ...
        if (prereqsMet && !Vehicle.CompUpgradeTree.NodeUnlocked(upgradeNode))
        {
  ...
  private void DrawVehicleGraphicComparison(Rect rect)
  {
    ...
    Rect arrowPointerRect = new(vehicleOriginalRect.xMax + 5,
      vehicleOriginalRect.y + vehicleOriginalRect.height / 2 - InfoPanelArrowPointerSize / 2,
      InfoPanelArrowPointerSize, InfoPanelArrowPointerSize);
    ...
    portraitBefore.Draw(vehicleOriginalRect, in requestBefore);
    ...
    {
      requestAfter.blitTargets.AddRange(renderTurrets.Where(turret => !turret.NoGraphic));
    }
    if (Vehicle.GetCachedComp<CompVehicleTurrets>() is { } compTurrets && !compTurrets.Turrets.NullOrEmpty())
    {
      requestAfter.blitTargets.AddRange(compTurrets.Turrets.Where(
        turret => !turret.NoGraphic && (excludeTurrets is null || !excludeTurrets.Contains(turret.key))));
    }
    portraitAfter.Draw(vehicleNewRect, in requestAfter);
