...
public class WITab_Vehicle_Manifest : WITab
{
  ...
  public override void OnOpen()
  {
    ...
    {
      SmashLog.ErrorOnce(
        $"{GetType()} used on world object not implementing <type>WorldObject</type>. This is not allowed, as inner vehicles are then unable to be fetched.",
        SelObject.GetHashCode());
      CloseTab();
    }
  }
  ...
  protected override void FillTab()
  {
    ...
    if (VehicleObject.CanDismount)
    {
      ...
      VehicleTabHelper_Passenger.ListPawns(ref curY, viewRect, scrollPosition, VehicleObject,
        string.Empty, VehicleObject.DismountedPawns.ToList(), ref moreDetailsForPawn);
    }
  ...
  private bool SectionLabel(Rect viewRect, ref float curY, string label,
    Texture2D configureButton = null)
  {
    return SectionLabel(viewRect, ref curY, label, Color.white, GenUI.MouseoverColor,
      configureButton: configureButton);
  }

  private bool SectionLabel(Rect viewRect, ref float curY, string label, Color baseColor,
    Color mouseoverColor, Texture2D configureButton = null)
  {
    ...
    Rect buttonRect = new Rect(labelRect.width - VehicleTabHelper_Passenger.PawnExtraButtonSize,
