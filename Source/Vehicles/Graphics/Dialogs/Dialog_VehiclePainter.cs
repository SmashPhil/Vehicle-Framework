...
public class Dialog_VehiclePainter : Window
{
  ...
  private void SetColor(string hexString)
  {
    if (HexToColor(hexString, out Color color))
    {
      SetColor(color);
    }
  }
  ...
  private void HandleDisplacementDrag(Rect rect)
  {
    if (selectedPattern.properties.dynamicTiling && Mouse.IsOver(rect))
    {
      ...
      if (Input.GetMouseButtonDown(0) && !draggingDisplacement)
      {
        ...
        float diffX = Mathf.InverseLerp(0f, rect.width, Event.current.mousePosition.x - rect.x);
        initialDragDifferenceX = diffX * 2 - 1 + displacementX;
        float diffY = Mathf.InverseLerp(rect.height, 0f, Event.current.mousePosition.y - rect.y);
        initialDragDifferenceY = diffY * 2 - 1 + displacementY;
      ...
      if (draggingDisplacement && Event.current.isMouse)
      {
        ...
        float diffX = Mathf.InverseLerp(0f, rect.width, Event.current.mousePosition.x - rect.x);
        displacementX = (initialDragDifferenceX - (diffX * 2 - 1)).Clamp(-1.5f, 1.5f);
        float diffY = Mathf.InverseLerp(rect.height, 0f, Event.current.mousePosition.y - rect.y);
        displacementY = (initialDragDifferenceY - (diffY * 2 - 1)).Clamp(-1.5f, 1.5f);
