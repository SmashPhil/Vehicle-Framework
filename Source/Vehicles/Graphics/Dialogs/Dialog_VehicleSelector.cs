...
public class Dialog_VehicleSelector : Window
{
  ...
  private List<VehiclePortrait> vehiclePortraits;
  private List<VehiclePortrait> vehicleDefPortraits;
  private bool closed;
  ...
  {
    foreach (VehiclePortrait portrait in vehiclePortraits)
    {
      portrait.MarkDirty();
    }
    foreach (VehiclePortrait portrait in vehicleDefPortraits)
    {
      portrait.MarkDirty();
    }
  }
  ...
  public override void PostOpen()
  {
    ...
    vehiclePortraits = new List<VehiclePortrait>(availableVehicles.Count);
    for (int i = 0; i < availableVehicles.Count; i++)
    {
      vehiclePortraits.Add(new VehiclePortrait());
    }
    vehicleDefPortraits = new List<VehiclePortrait>(availableVehicleDefs.Count);
    for (int i = 0; i < availableVehicleDefs.Count; i++)
    {
      vehicleDefPortraits.Add(new VehiclePortrait());
    }
  ...
  public override void PostClose()
  {
    ...
    vehiclePortraits.Clear();

    foreach (VehiclePortrait portrait in vehicleDefPortraits)
    {
      portrait.Dispose();
    }
    vehicleDefPortraits.Clear();
    closed = true;
  }

  public override void DoWindowContents(Rect inRect)
  {
    // RimWorld bug causes DoWindowContents to call for 1 more event type after PostClose.
    // VehiclePortraits will be disposed by this point so we need to skip this cycle.
    if (closed)
      return;
  ...
  private void DrawVehicles(Rect rect)
  {
    ...
    while (curVehicle < vehicleCount)
    {
      ...
      for (int j = 0; j < Columns && curVehicle < vehicleCount; j++)
      {
        ...
        if (showVehicleDefs)
        {
          VehiclePortrait portrait = vehicleDefPortraits[curVehicle];
          VehicleDef vehicleDef = availableVehicleDefs[curVehicle];
          DrawVehicleDef(labelRect, iconRect, vehicleDef, portrait);
        }
        else
        {
          VehiclePortrait portrait = vehiclePortraits[curVehicle];
          VehiclePawn vehicle = availableVehicles[curVehicle];
          DrawVehicle(labelRect, iconRect, vehicle, portrait);
        }
        curX = labelRect.xMax;
        curVehicle++;
      }
  ...
  private void DrawVehicle(Rect rowRect, Rect iconRect, VehiclePawn vehicle, VehiclePortrait portrait)
  {
    ...
    portrait.Draw(iconRect, BlitRequest.For(vehicle));
  ...
  private void DrawVehicleDef(Rect rowRect, Rect iconRect, VehicleDef vehicleDef, VehiclePortrait portrait)
  {
    const float CheckboxSize = 24;

    Vector2 texProportions = vehicleDef.graphicData.drawSize;
    (texProportions.x, texProportions.y) = (texProportions.y, texProportions.x);
    portrait.Draw(iconRect, BlitRequest.For(vehicleDef));
