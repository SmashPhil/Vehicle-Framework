...
public partial class VehicleTurret
{
  ...
  IEnumerable<RenderData> IBlitTarget.GetRenderData(Rect rect, BlitRequest request)
  {
      ...
      BlitRequest request, Graphic_Turret graphic)
    {
      ...
      {
        material = RGBMaterialPool.GetUi(target, Rot8.North);
      ...
      {
        if (IsFeatureEnabled(BlitTexturePortraits) || graphic.Shader.SupportsMaskTex())
        {
          material = graphic.MatAtFull(Rot8.North);
        }
        else
        {
          material = null;
