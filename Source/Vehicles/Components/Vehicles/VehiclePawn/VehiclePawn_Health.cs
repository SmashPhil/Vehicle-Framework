...
public partial class VehiclePawn
{
  ...
  public override void Destroy(DestroyMode mode = DestroyMode.Vanish)
  {
    ...
    // Pawn::Destroy logs a pointless error about "unsupported" DestroyModes, so we have to convert
    // ours to a "supported" one before calling to base. There are no obvious side effects for
    // using an unsupported one but transpiling out that top error is more problematic.
    if (Spawned)
    {
      inventory.DropAllNearPawn(Position);

      using var scope = GlobalObjectPool.Get(out List<Thing> leavings);
      GenLeaving.DoLeavingsFor(this, Map, mode, leavings);
      this.TryGetComp<CompExplosive>()?.AddThingsIgnoredByExplosion(leavings);
      Notify_KilledLeavingsLeft(leavings);
    }
    ...
    // Pawn::Destroy checks game over condition, but we need to redo this after deregistering from
