...
public sealed class VehicleCaravan_PathFollower : IExposable
{
  ...
  [Obsolete]
  public static int CostToMove(VehicleCaravan caravan, PlanetTile start, PlanetTile end, int? ticksAbs)
  {
  ...
  {
    var data = new TicksPerMoveData { tile = start, nextTile = end, ticksPerMove = caravan.TicksPerMove };
    return CostToMove(caravan.VehiclesListForReading, in data);
  }

  // TODO 1.7 - Switch to TicksPerMoveData
  [Obsolete]
  ...
  {
    var data = new TicksPerMoveData { tile = start, nextTile = end, ticksAbs = ticksAbs, explanation = explanation };
    return CostToMove(vehicleDefs, in data);
  }

  [Profile]
  public static int CostToMove(List<VehicleDef> vehicleDefs, ref readonly TicksPerMoveData data)
  {
    PlanetTile start = data.tile;
    PlanetTile end = data.nextTile;
    if (start == end)
      return 0;

    StringBuilder explanation = data.explanation;
    explanation?.AppendLine(data.caravanTicksPerMoveExplanation);
    if (explanation != null)
    {
    ...
    string tileExplanation = null;
    foreach (VehicleDef vehicleDef in vehicleDefs)
    {
      using var tileSbScope = GlobalObjectPool.Get(out StringBuilder tileStringBuilder);
      float newCost = WorldVehiclePathGrid.CalculatedMovementDifficultyAt(end, vehicleDef,
        explanation != null ? tileStringBuilder : null);
      if (newCost < cost)
      {
        cost = newCost;
        tileExplanation = tileStringBuilder.ToString();
      }
    }
    explanation?.AppendLine(tileExplanation.Indented("  "));

    float winterOffset =
      WinterPathingHelper.GetCurrentWinterMovementDifficultyOffset(vehicleDefs, end, explanation);
    cost += winterOffset;

    RoadCostHelper.RoadMultiplier roadSummary =
      RoadCostHelper.GetRoadMovementMultiplier(vehicleDefs, start, end);
    if (explanation != null)
    {
      if (roadSummary.HasMultiplier)
      {
        explanation.AppendLine($"  {roadSummary.Explanation}");
      }
      explanation.AppendLine($"  = {cost * roadSummary.multiplier:0.##}");
    }

    int finalTicksPerMove = Mathf.RoundToInt(data.ticksPerMove * cost * roadSummary.multiplier);
    finalTicksPerMove = Mathf.Clamp(finalTicksPerMove, 1, MaxMoveTicks);
    if (explanation != null)
    {
      explanation.AppendLine();
      explanation.AppendLine("FinalCaravanMovementSpeed".Translate() + ":");
      float preProcessSpeed = GenDate.TicksPerDay / (float)data.ticksPerMove;
      float finalCost = cost * roadSummary.multiplier;
      explanation.Append(
        $"  {preProcessSpeed:0.#} / {finalCost:0.##} = " +
        $"{GenDate.TicksPerDay / finalTicksPerMove:0.##} {"TilesPerDay".Translate()}");
    }
    return finalTicksPerMove;
  }
  ...
  {
    var data = new TicksPerMoveData { tile = start, nextTile = end, ticksAbs = ticksAbs, explanation = explanation };
    return CostToMove(vehicles, in data);
  }

  [Profile]
  public static int CostToMove(List<VehiclePawn> vehicles, ref readonly TicksPerMoveData data)
  {
    PlanetTile start = data.tile;
    PlanetTile end = data.nextTile;
    if (start == end)
    ...
    {
      using var tileSbScope = GlobalObjectPool.Get(out StringBuilder tileStringBuilder);
      float newCost = WorldVehiclePathGrid.CalculatedMovementDifficultyAt(start, vehicle.VehicleDef,
        explanation != null ? tileStringBuilder : null);
      if (newCost < cost)
      {
        cost = newCost;
    ...
    float winterOffset =
      WinterPathingHelper.GetCurrentWinterMovementDifficultyOffset(vehicles, start, stringBuilder);
    cost += winterOffset;

    RoadCostHelper.RoadMultiplier roadSummary = RoadCostHelper.GetRoadMovementMultiplier(vehicles, start, end);
    if (explanation != null)
    {
      if (roadSummary.HasMultiplier)
      {
    ...
    int finalTicksPerMove = (int)(data.ticksPerMove * cost * roadSummary.multiplier);
    finalTicksPerMove = Mathf.Clamp(finalTicksPerMove, 1, MaxMoveTicks);
    if (explanation != null)
    {
      explanation.AppendLine();
