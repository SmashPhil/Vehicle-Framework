...
public class FlakWorker : AntiAircraftWorker
{
  ...
  public virtual int CooldownRange => Mathf.CeilToInt(Rand.Range(
    airDefense.antiAircraftDef.ticksBetweenShots * 0.75f,
    airDefense.antiAircraftDef.ticksBetweenShots * 1.25f));

  public override bool ShouldDrawSearchLight => airDefense.parent is MapParent mapParent &&
    (mapParent.Map is null || !turretsCachedInMap.NullOrEmpty());
    ...
    get
    {
      if (spawnLocations.NullOrEmpty())
      {
        IEnumerable<Vector3> positions =
          Building_Artillery.RandomWorldPosition(airDefense.parent.Tile,
            airDefense.defenseBuildings);
