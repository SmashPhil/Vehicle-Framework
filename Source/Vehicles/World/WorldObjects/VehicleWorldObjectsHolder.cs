...
public class VehicleWorldObjectsHolder : WorldComponent
{
  ...
  {
    foreach (VehicleCaravan caravan in VehicleCaravans)
    {
      if (caravan.VehiclesListForReading.Contains(vehicle))
        return caravan;
    }
    return null;
  }
  ...
  {
    foreach (StashedVehicle stash in StashedVehicles)
    {
      if (stash.Vehicles.Contains(vehicle))
        return stash;
    }
    return null;
  }
