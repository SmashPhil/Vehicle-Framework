	...
	public virtual IEnumerable<ArrivalOption> GetArrivalOptions(GlobalTargetInfo target)
	{
		...
		if (target.WorldObject is MapParent mapParent)
		{
				...
				yield return new ArrivalOption("LandInExistingMap".Translate(mapParent.Label),
					continueWith: delegate (TargetData<GlobalTargetInfo> targetData)
					{
						...
						LandingTargeter.Instance.BeginTargeting(vehicle, mapParent.Map,
							...
							{
								StartTargetingLocalMap(vehicle, targetData, mapParent, landingCell, rot);
							},
							allowRotating: vehicle.VehicleDef.rotatable,
							targetValidator: delegate (LocalTargetInfo targetInfo)
							{
								return targetInfo.Cell.InBounds(mapParent.Map) &&
			...
			else if (!mapParent.HasMap)
			{
				...
				// Escape ship has special handling as a non-event map that gets automatically claimed by the player upon arrival.
				// This is an exception for non-vanilla behavior where we can allow the player to land inside the map rather than
				// form some intermediate caravan on the world map. I'm not sure why the Odyssey shuttle doesn't.
