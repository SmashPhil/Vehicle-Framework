...
public static class VehicleArrivalActionUtility
{
  public static IEnumerable<FloatMenuOption> GetFloatMenuOptions<T>(
    Func<FloatMenuAcceptanceReport> acceptanceReportGetter, Func<T> arrivalActionGetter,
    string label, VehiclePawn vehicle, int destinationTile,
    Action<Action> uiConfirmationCallback = null) where T : AerialVehicleArrivalAction
  {
    ...
    {
      yield return new FloatMenuOption(label + " (" + floatMenuAcceptanceReport.FailReason + ")",
        null);
      yield break;
    }
    yield return new FloatMenuOption(label, delegate
    {
      ...
      if (floatMenuAcceptanceReport2.Accepted)
      {
        ...
        {
          if (!vehicle.Spawned &&
            AerialVehicleLaunchHelper.GetOrMakeAerialVehicle(vehicle) is AerialVehicleInFlight
              aerialVehicle)
          {
      ...
      {
        Messages.Message(floatMenuAcceptanceReport2.FailMessage,
