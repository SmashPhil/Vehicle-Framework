  ...
  public override IEnumerable<UpgradeTextEntry> UpgradeDescription(VehiclePawn vehicle)
  {
      ...
      foreach (StatDefUpgrade statDefUpgrade in stats)
      {
        switch (statDefUpgrade.type)
        {
          case UpgradeType.Add:
            yield return new UpgradeTextEntry(statDefUpgrade.def.LabelCap, statDefUpgrade.ValueFormatted,
              statDefUpgrade.value, UpgradeEffectType.Positive);
          break;
    ...
    if (!vehicleStats.NullOrEmpty())
    {
      foreach (VehicleStatDefUpgrade vehicleStatDefUpgrade in vehicleStats)
      {
        ...
        {
          case UpgradeType.Add:
            yield return new UpgradeTextEntry(vehicleStatDefUpgrade.def.LabelCap, vehicleStatDefUpgrade.ValueFormatted,
              vehicleStatDefUpgrade.value, vehicleStatDefUpgrade.def.upgradeEffectType);
          break;
          case UpgradeType.Set:
            yield return new UpgradeTextEntry(vehicleStatDefUpgrade.def.LabelCap,
              vehicleStatDefUpgrade.value.ToString());
          break;
        }
      }
    }
    if (!statCategories.NullOrEmpty())
    {
      foreach (StatCategoryUpgrade statCategoryUpgrade in statCategories)
      {
        ...
        {
          case UpgradeType.Add:
            yield return new UpgradeTextEntry(statCategoryUpgrade.def.LabelCap, statCategoryUpgrade.ValueFormatted,
              statCategoryUpgrade.value, statCategoryUpgrade.def.upgradeEffectType);
          break;
          case UpgradeType.Set:
