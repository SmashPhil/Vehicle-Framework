  ...
  public class VehicleTargetReservation : Reservation<LocalTargetInfo>
  {
    ...
    public override void VerifyAndValidateClaimants()
    {
      ...
      foreach (Pawn actor in actors)
      {
        ...
        if (actor == null || !actor.Spawned || actor.Dead || actor.CurJob.def != jobDef ||
          actor.CurJob.targetA != targetA || !claimants[actor].IsValid || actor.Drafted || vehicle.Drafted)
        {
    ...
    {
      base.ExposeData();
      Scribe_Collections.Look(ref claimants, nameof(claimants), LookMode.Reference, LookMode.LocalTargetInfo,
        ref pawnClaimants, ref pawnTargets);
    }
  }
}