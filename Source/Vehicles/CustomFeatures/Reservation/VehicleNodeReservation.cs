  ...
  public class VehicleNodeReservation : Reservation<ThingDefCountClass>
  {
      ...
      StringBuilder stringBuilder = null)
    {
      return !claimants.ContainsKey(pawn) && claimants.Count < maxClaimants &&
        vehicle.CompUpgradeTree.Upgrading && MaterialsLeft().NotNullAndAny();
    }
    ...
    public List<ThingDefCountClass> MaterialsLeft()
    {
      ...
      List<ThingDefCountClass> tdList = [];
      foreach ((ThingDef thingDef, int count) in cachedTd)
      {
        tdList.Add(new ThingDefCountClass(thingDef, count));
      }
      return tdList;
    }
