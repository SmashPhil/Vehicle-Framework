...
namespace Vehicles
{
	public class WaterRegionAndRoomUpdater
	{
		...
		public void RebuildAllWaterRegions()
		{
			if (!Enabled)
			{
				Log.Warning("Called RebuildAllRegions but WaterRegionAndRoomUpdater is disabled. Regions won't be rebuilt.");
			}
		...
		private void CreateOrAttachToExistingRooms(int numRegionGroups)
		{
			for (int i = 0; i < numRegionGroups; i++)
			{
				...
				{
					WaterRoom room2 = FindCurrentRegionGroupNeighborWithMostRegions(out bool flag);
					if (room2 is null)
					{
