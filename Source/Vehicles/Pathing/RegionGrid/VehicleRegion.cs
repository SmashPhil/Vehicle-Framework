...
namespace Vehicles
{
	...
	public sealed class VehicleRegion
	{
		...
		private readonly ConcurrentSet<(Pawn cachedFor, Danger danger)> cachedDangers = new ConcurrentSet<(Pawn, Danger)>();
		...
		public Danger DangerFor(Pawn pawn)
		{
			if (Current.ProgramState == ProgramState.Playing)
			{
				...
				{
					foreach ((Pawn cachedFor, Danger danger) in cachedDangers.Keys)
					{
						if (cachedFor == pawn)
						{
							return danger;
