...
public sealed class VehicleReachability : VehicleGridManager
{
  ...
  private readonly VehicleReachabilityCache cache = new();

  private VehiclePathGrid pathGrid;
  private VehicleRegionGridManager regionGridManager;

  public VehicleReachability(VehiclePathingSystem mapping, VehicleDef createdFor) : base(mapping, createdFor)
  {
    ...
    TraverseParms traverseParms)
  {
    ...
    dest = (LocalTargetInfo)GenPathVehicles.ResolvePathMode(vehicleDef, mapping.map, dest.ToTargetInfo(mapping.map), 
      ref peMode);
    ...
    try
    {
      ...
      RegionGridType gridType = VehicleRegionGridManager.GetGridType(traverseParms);
      destRegions.RemoveDuplicates();
      openQueue.Clear();
      DetermineStartRegions(start, gridType);
      ...
      if (traverseParms.mode is TraverseMode.PassAllDestroyableThings or TraverseMode.PassAllDestroyableThingsNotWater
          or TraverseMode.NoPassClosedDoorsOrWater)
      {
  ...
  private bool CheckRegionBasedReachability(TraverseParms traverseParms)
  {
    ...
    bool RegionReachable(VehicleRegion linkedRegion)
    {
      ...
      FlashRect(mapping.map, [..linkedRegion.Cells]);
      return false;
    }

    static void FlashRect(Map map, List<IntVec3> cells)
    {
    ...
    TraverseParms traverseParms)
  {
    ...
    VehicleRegionGrid regionGrid = regionGridManager[VehicleRegionGridManager.GetGridType(traverseParms)];
  ...
  private class AStar
  {
    ...
    {
      startingRegion = null;//vehicleReachability.regionGrid.GetValidRegionAt(start);
      ...
      destinationRegion = VehicleRegionAndRoomQuery.RegionAt(dest.Cell, mapping,
        vehicleReachability.createdFor);
