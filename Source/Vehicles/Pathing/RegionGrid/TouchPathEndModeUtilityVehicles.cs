...
public static class TouchPathEndModeUtilityVehicles
{
  ...
  /// </summary>
  public static bool IsCornerTouchAllowed(int cornerX, int cornerZ, int adjCardinal1X,
    int adjCardinal1Z, int adjCardinal2X, int adjCardinal2Z, Map map, VehicleDef vehicleDef)
  {
    ...
    IntVec3 intVec = new(adjCardinal1X, 0, adjCardinal1Z);
    IntVec3 intVec2 = new(adjCardinal2X, 0, adjCardinal2Z);
    VehiclePathingSystem mapping = map.GetCachedMapComponent<VehiclePathingSystem>();
    return (mapping[vehicleDef].VehiclePathGrid.Walkable(intVec) &&
      intVec.GetDoor(map) is null) || (mapping[vehicleDef].VehiclePathGrid.Walkable(intVec2) &&
      intVec2.GetDoor(map) is null);
  }
  ...
  /// </summary>
  public static bool IsAdjacentCornerAndNotAllowed(IntVec3 cell, IntVec3 BL, IntVec3 TL,
    IntVec3 TR, IntVec3 BR, Map map, VehicleDef vehicleDef)
  {
    return (cell == BL && !IsCornerTouchAllowed(BL.x + 1, BL.z + 1, BL.x + 1, BL.z, BL.x,
        BL.z + 1, map, vehicleDef)) ||
      (cell == TL && !IsCornerTouchAllowed(TL.x + 1, TL.z - 1, TL.x + 1, TL.z, TL.x, TL.z - 1,
        map, vehicleDef)) ||
      (cell == TR && !IsCornerTouchAllowed(TR.x - 1, TR.z - 1, TR.x - 1, TR.z, TR.x, TR.z - 1,
        map, vehicleDef)) ||
      (cell == BR && !IsCornerTouchAllowed(BR.x - 1, BR.z + 1, BR.x - 1, BR.z, BR.x, BR.z + 1,
        map, vehicleDef));
  }
  ...
  /// </summary>
  public static bool IsAdjacentOrInsideAndAllowedToTouch(IntVec3 root, LocalTargetInfo target,
    Map map, VehicleDef vehicleDef)
  {
    GenAdj.GetAdjacentCorners(target, out IntVec3 b1, out IntVec3 t1, out IntVec3 tr,
      out IntVec3 br);
    return root.AdjacentTo8WayOrInside(target) &&
      !IsAdjacentCornerAndNotAllowed(root, b1, t1, tr, br, map, vehicleDef);
  }
}