...
public static class CaravanHelper
{
		...
		bool sendMessage = true)
  {
	...
	/// <summary>
	/// Create <see cref="VehicleCaravan"/> on World Map and move pawns from <paramref name="caravan"/> to new <see cref="VehicleCaravan"/> instance.
	/// </summary>
	public static VehicleCaravan SwapToVehicleCaravan(Caravan caravan)
	{
		if (!caravan.PawnsListForReading.ContainsAny(IsVehicle))
		{
		...
		{
			if (pawn is VehiclePawn caravanVehicle)
			{
				vehicle = caravanVehicle;
			}
			else
			{
				pawns.Add(pawn);
		...
		caravan.Destroy();
		RoleHelper.Distribute([vehicle], pawns);
		VehicleCaravan vehicleCaravan = MakeVehicleCaravan([vehicle, .. pawns], Faction.OfPlayer, tile, true);
	...
	public static VehiclePawn UsableVehicleWithTheMostFreeSpace(Pawn pawn)
	{
		...
		float maxFreeSpace = 0f;
		foreach (Pawn candidate in candidates)
		{
			if (candidate is VehiclePawn vehicle && vehicle != pawn &&
				(pawn.InVehicleCaravan() || pawn.CanReach(vehicle, PathEndMode.Touch, Danger.Deadly)))
			{
				float freeSpace = MassUtility.FreeSpace(candidate);
				if (bestVehicle is null || freeSpace > maxFreeSpace)
				{
					bestVehicle = vehicle;
