...
public static class CaravanFormation
{
  ...
  private static bool CheckForErrors()
  {
    ...
    bool autonomous = formation.vehicles.All(Ext_Vehicles.IsAutonomousVehicle);
    if (!autonomous && !formation.pawns.Any(pawn =>
    ...
    VehiclePawn unassignedVehicle = formation.vehicles.FirstOrDefault(static vehicle =>
      vehicle.CountAssignedToVehicle() < vehicle.PawnCountToOperate);
    if (unassignedVehicle != null)
    {
      Messages.Message("VF_NotEnoughCrew".Translate(unassignedVehicle), MessageTypeDefOf.RejectInput, false);
      return false;
    }
