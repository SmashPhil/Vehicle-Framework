...
public static class VehicleCaravanPathingHelper
{
  ...
  public static bool ShouldRestAt(VehicleCaravan caravan, int tile)
  {
    ...
    bool conditionalResting = !caravan.vehiclePather.Moving ||
  ...
  public static int EstimatedTicksToArrive(VehicleCaravan caravan, bool allowCaching)
  {
    if (allowCaching && caravan == cachedForCaravan &&
      ...
      Find.TickManager.TicksGame - cacheTicks < 100)
    {
      return cachedResult;
    }
    int to = -1;
  ...
  public static int EstimatedTicksToArrive(PlanetTile from, PlanetTile to, VehicleCaravan caravan)
  {
    int result;
    using (WorldPath worldPath =
      Find.World.GetComponent<WorldVehiclePathfinder>().FindPath(from, to, caravan))
    {
      if (!worldPath.Found)
      {
        result = 0;
      ...
      {
        List<VehicleDef> vehicleDefs = caravan.VehiclesListForReading
         .Select(vehicle => vehicle.VehicleDef).ToList();
        result = EstimatedTicksToArrive(vehicleDefs, from, to, worldPath, 0f,
          (caravan != null) ? caravan.TicksPerMove : 3300, Find.TickManager.TicksAbs);
      }
    }
    return result;
  }

  public static int EstimatedTicksToArrive(List<VehicleDef> vehicleDefs, int from, int to,
  ...
  public static void EstimatedTicksToArriveToEvery(List<VehicleDef> vehicleDefs, int from, int to,
    WorldPath path, float nextTileCostLeft, int caravanTicksPerMove, int curTicksAbs,
    List<(int, int)> outTicksToArrive)
  {
    ...
    int pathSteps = 0;
    int restDuration =
      GenDate.TicksPerDay / 3 - 1; //Accounts for resting time in caravans that must rest
    int movementDuration = GenDate.TicksPerDay - restDuration;
    int ticksToMove = 0;
    int nonRestTicks;
    if (ShouldRestAt(vehicleDefs, from) &&
      CaravanNightRestUtility.WouldBeRestingAt(from, curTicksAbs))
    {
      ...
      {
        int num8 = Mathf.CeilToInt(GetCostToMove(vehicleDefs, nextTileCostLeft, path.Peek(0) == to,
          curTicksAbs, result, caravanTicksPerMove, from, to) / 1f);
        if (num8 <= 10000)
        {
          result += num8;
    ...
    Log.ErrorOnce("Could not calculate estimated ticks to arrive. Too many iterations.",
      1837451324);
    outTicksToArrive.Add((to, result));
  }

  private static float GetCostToMove(List<VehicleDef> vehicleDefs, float initialNextTileCostLeft,
    bool firstInPath, int initialTicksAbs, int curResult, int caravanTicksPerMove, int curTile,
    int nextTile)
  {
    if (firstInPath)
    {
    ...
    return VehicleCaravan_PathFollower.CostToMove(vehicleDefs, caravanTicksPerMove, curTile,
      nextTile, ticksAbs: value);
  }

  public static int EstimatedTicksToArrive(int destinationTile,
    List<(int, int)> estimatedTicksToArriveToEvery)
  {
  ...
  public static int TileIllBeInAt(int ticksAbs, List<(int, int)> estimatedTicksToArriveToEvery,
    int ticksAbsUsedToCalculateEstimatedTicksToArriveToEvery)
  {
    ...
    {
      int num = ticksAbsUsedToCalculateEstimatedTicksToArriveToEvery +
        estimatedTicksToArriveToEvery[i].Item2;
      if (ticksAbs >= num)
      {
