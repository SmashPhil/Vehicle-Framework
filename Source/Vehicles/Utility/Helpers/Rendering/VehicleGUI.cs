...
{
  private const float OversampleFactor = 1.25f;
  ...
  public static RenderTextureBuffered CreateRenderTextureBuffer(Rect rect, in BlitRequest request,
    float oversampleFactor = OversampleFactor)
  {
    (int width, int height) = GetOptimalTextureSize(rect, in request, oversampleFactor);
    ...
    return new RenderTextureBuffered(rtA, rtB);
  }
    ...
    Rot8? rot = null)
  {
    ...
    Rect adjustedRect = VehicleGraphics.AdjustRectToVehicleDef(vehicleDef, rect, rotDrawn);
  ...
  {
    RenderTextureDrawer.Init(renderTexture);
    try
    {
      foreach (IBlitTarget blitTarget in request.blitTargets)
      {
    ...
    GizmoRenderParms parms, VehicleBuildDef buildDef)
  {
    ...
    PatternData defaultPatternData = new(VehicleMod.settings.vehicles.defaultGraphics.TryGetValue(vehicleDef.defName,
          vehicleDef.graphicData));
    if (command.Disabled)
    {
    ...
    vehicleDef.Portrait.Draw(buttonRect, in request);
