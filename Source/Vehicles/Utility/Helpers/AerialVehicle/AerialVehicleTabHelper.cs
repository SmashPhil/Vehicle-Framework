...
public static class AerialVehicleTabHelper
{
  ...
  public static void DoRows(Vector2 size, List<TransferableImmutable> things,
    AerialVehicleInFlight aerialVehicle, ref Vector2 scrollPosition, ref float scrollViewHeight)
  {
  ...
  public static Vector2 GetSize(List<TransferableImmutable> things, float paneTopY,
    bool doNeeds = true)
  {
  ...
  private static void DoRow(ref float curY, Rect viewRect, Rect scrollOutRect,
    Vector2 scrollPosition, TransferableImmutable thing, AerialVehicleInFlight aerialVehicle)
  {
  ...
  private static void DoRow(Rect rect, TransferableImmutable thing,
  ...
  public static void DoAbandonButton(Rect rowRect, Thing thing, AerialVehicleInFlight aerialVehicle)
  {
    ...
    if (Mouse.IsOver(rect))
    {
      TooltipHandler.TipRegion(rect,
        () => AerialVehicleAbandonOrBanishHelper.GetAbandonOrBanishButtonTooltip(thing, false),
        Gen.HashCombineInt(thing.GetHashCode(), 1383004931));
    }
  }

  public static void DoAbandonButton(Rect rowRect, TransferableImmutable transferable,
    AerialVehicleInFlight aerialVehicle)
  {
    Rect rect = new Rect(rowRect.width - AbandonButtonSize,
      (rowRect.height - AbandonButtonSize) / 2f, AbandonButtonSize, AbandonButtonSize);
    if (Widgets.ButtonImage(rect, CaravanThingsTabUtility.AbandonButtonTex, true))
    {
      AerialVehicleAbandonOrBanishHelper.TryAbandonOrBanishViaInterface(transferable,
        aerialVehicle);
    }
    if (Mouse.IsOver(rect))
    {
      TooltipHandler.TipRegion(rect,
        () => AerialVehicleAbandonOrBanishHelper.GetAbandonOrBanishButtonTooltip(transferable,
          false), Gen.HashCombineInt(transferable.GetHashCode(), 8476546));
    }
  }

  public static void DoAbandonSpecificCountButton(Rect rowRect, Thing thing,
    AerialVehicleInFlight aerialVehicle)
  {
    Rect rect = new Rect(rowRect.width - AbandonSpecificCountButtonSize,
      (rowRect.height - AbandonSpecificCountButtonSize) / 2f, AbandonSpecificCountButtonSize,
      AbandonSpecificCountButtonSize);
    ...
    {
      TooltipHandler.TipRegion(rect,
        () => AerialVehicleAbandonOrBanishHelper.GetAbandonOrBanishButtonTooltip(thing, true),
        Gen.HashCombineInt(thing.GetHashCode(), 1163428609));
    }
  }

  public static void DoAbandonSpecificCountButton(Rect rowRect, TransferableImmutable transferable,
    AerialVehicleInFlight aerialVehicle)
  {
    Rect rect = new Rect(rowRect.width - AbandonButtonSize,
      (rowRect.height - AbandonButtonSize) / 2f, AbandonButtonSize, AbandonButtonSize);
    if (Widgets.ButtonImage(rect, CaravanThingsTabUtility.AbandonSpecificCountButtonTex, true))
    {
      AerialVehicleAbandonOrBanishHelper.TryAbandonSpecificCountViaInterface(transferable,
        aerialVehicle);
    }
    if (Mouse.IsOver(rect))
    {
      TooltipHandler.TipRegion(rect,
        () => AerialVehicleAbandonOrBanishHelper
         .GetAbandonOrBanishButtonTooltip(transferable, true),
        Gen.HashCombineInt(transferable.GetHashCode(), 1163428609));
    }
  }

  public static void DoOpenSpecificTabButton(Rect rowRect, Pawn p, ref Pawn specificTabForPawn)
  {
    Color baseColor = (p == specificTabForPawn) ?
      CaravanThingsTabUtility.OpenedSpecificTabButtonColor :
      Color.white;
    Color mouseoverColor = (p == specificTabForPawn) ?
      CaravanThingsTabUtility.OpenedSpecificTabButtonMouseoverColor :
      GenUI.MouseoverColor;
    Rect rect = new Rect(rowRect.width - SpecificTabButtonSize,
      (rowRect.height - SpecificTabButtonSize) / 2f, SpecificTabButtonSize, SpecificTabButtonSize);
  ...
  public static void DoOpenSpecificTabButtonInvisible(Rect rect, Pawn pawn,
  ...
  public static void DrawMass(TransferableImmutable transferable, Rect rect)
  {
    ...
    {
      num += transferable.things[i].GetStatValue(StatDefOf.Mass, true, -1) *
        transferable.things[i].stackCount;
    }
    DrawMass(num, rect);
  }
