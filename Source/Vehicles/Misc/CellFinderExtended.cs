...
public static class CellFinderExtended
{
	...
	public static IntVec3 MiddleEdgeCell(Rot4 dir, Map map, Pawn pawn, Predicate<IntVec3> validator)
	{
		...
		{
			return map.terrainGrid.TerrainAt(x).IsRiver;
		}
	}
		...
		out IntVec3 result, int maxRegions = 999999)
	{
		...
		VehicleRegion region = VehicleRegionAndRoomQuery.RegionAt(root, map, vehicleDef);
		if (region == null)
		{
			result = IntVec3.Invalid;
			return false;
		...
		{
			return to.Allows(traverseParms)
				&& (radius > 1000f || to.extentsClose.ClosestDistSquaredTo(root) <= radSquared)
				&& (regionValidator == null || regionValidator(to));
		}

		bool RegionProcessor(VehicleRegion regionToProcess)
		{
			workingRegions.Add(regionToProcess);
			return false;
		}
	}
		...
		int radius, out IntVec3 result, Predicate<IntVec3> validator = null)
	{
		return TryFindRandomReachableCellNear(root, map, vehicleDef, radius,
			TraverseParms.For(TraverseMode.NoPassClosedDoors), validator, null,
			out result);
	}
		...
		Predicate<IntVec3> validator, bool waterEntry = false, int firstTryWithRadius = 4)
	{
		...
		if (pawn is VehiclePawn vehicle)
		{
			...
			for (int i = 0; i < 3; i++)
			{
				if (waterEntry)
				{
					if (TryFindRandomReachableCellNear(root, map, vehicle.VehicleDef, num,
						...
						cell => validator(cell) && vehicle.CellRectStandable(map, cell) &&
							(root.Fogged(map) || !cell.Fogged(map)) && cell.GetFirstPawn(map) is null, null,
						out result))
					{
						return result;
					}
			...
			while (!TryRandomClosewalkCellNear(root, map, vehicle.VehicleDef, num, out result))
			{
