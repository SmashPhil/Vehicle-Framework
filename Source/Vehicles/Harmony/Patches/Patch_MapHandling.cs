...
internal class Patch_MapHandling : IPatchCategory
{
  ...
  private static void DebugUpdateVehicleRegions()
  {
    if (Find.CurrentMap is { } map && !WorldRendererUtility.WorldRendered &&
      DebugHelper.AnyDebugSettings && DebugHelper.Local.VehicleDef is { } vehicleDef)
    {
      map.GetCachedMapComponent<VehiclePathingSystem>()[vehicleDef].VehicleRegionGridManager[RegionGridType.Normal]
        .DebugDraw(DebugHelper.Local.DebugType);
    }
  }

  private static void DebugOnGUIVehicleRegions()
  {
    if (Find.CurrentMap is { } map && !WorldRendererUtility.WorldRendered &&
        DebugHelper.AnyDebugSettings && DebugHelper.Local.VehicleDef is { } vehicleDef)
    {
      map.GetCachedMapComponent<VehiclePathingSystem>()[vehicleDef].VehicleRegionGridManager[RegionGridType.Normal]
        .DebugOnGUI(DebugHelper.Local.DebugType);
    }
  }
}