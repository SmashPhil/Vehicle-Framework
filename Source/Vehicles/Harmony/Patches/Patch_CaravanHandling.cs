...
internal class Patch_CaravanHandling : IPatchCategory
{
  ...
  void IPatchCategory.PatchMethods()
  {
    ...
    HarmonyPatcher.Patch(original: AccessTools.Method(typeof(Caravan), nameof(Caravan.Notify_PawnAdded)),
      postfix: new HarmonyMethod(typeof(Patch_CaravanHandling),
        nameof(NotifyVehicleCaravanPawnAdded)));
    HarmonyPatcher.Patch(original: AccessTools.Method(typeof(Caravan), nameof(Caravan.Notify_PawnRemoved)),
      postfix: new HarmonyMethod(typeof(Patch_CaravanHandling),
        nameof(NotifyVehicleCaravanPawnRemoved)));
    ...
    HarmonyPatcher.Patch(original: AccessTools.Method(typeof(Pawn), nameof(Pawn.PreTraded)),
      postfix: new HarmonyMethod(typeof(Patch_CaravanHandling), nameof(RemoveSoldPawnFromVehicle)));
    HarmonyPatcher.Patch(
      original: AccessTools.Method(typeof(Settlement_TraderTracker),
  ...
  private static bool DoPeopleAnimalsAndVehicle(Rect inRect, ref float curY,
  ...
  private static bool EnterMapVehiclesCatchAll2(Caravan caravan, Map map,
    CaravanDropInventoryMode dropInventoryMode = CaravanDropInventoryMode.DoNotDrop,
  ...
  {
    if (__instance is not VehicleCaravan caravan)
      return true;

    if (caravan.IsAutonomousCaravan())
    {
      __result = false;
      return false;
    }
  ...
  private static bool NoRestForVehicles(Caravan __instance, ref bool __result)
  {
    ...
    {
      __result = !caravan.IsAutonomousCaravan() && VehicleCaravanPathingHelper.ShouldRestAt(caravan, caravan.Tile);
      return false;
    }
    return true;
  }
  ...
  private static bool VehicleCaravanTicksPerMoveExplanation(ref string __result,
    Caravan __instance)
  {
    if (__instance is VehicleCaravan vehicleCaravan)
    {
  ...
  private static bool GetBaseForagedNutritionPerDayInVehicle(Pawn p, out bool skip,
    ref float __result)
  {
    skip = false;
  ...
  private static bool AerialVehicleInventoryItems(Pawn playerNegotiator,
  ...
  private static bool FindVehicleToMoveInventoryTo(ref Pawn __result, Thing item, List<Pawn> candidates,
  ...
  private static bool FindBestNegotiatorInVehicle(Caravan caravan, ref Pawn __result,
  ...
  private static void RemoveSoldPawnFromVehicle(Pawn __instance, TradeAction action)
  {
    switch (action)
    {
      ...
      {
        if (__instance.GetVehicle() is { } vehicle)
          vehicle.DisembarkPawn(__instance);
      }
      break;
  ...
  private static void MergeCaravansWithVehicle(List<Caravan> caravans)
  {
    ...
    if (survivor.pawns.InnerListForReading.Exists(IsVehicle) && survivor is not VehicleCaravan)
    {
      CaravanHelper.SwapToVehicleCaravan(survivor);
    }
    return;
    ...
    Faction faction, PlanetTile startingTile, bool addToWorldPawnsIfNotAlready)
  {
    ...
    {
      __result = CaravanHelper.MakeVehicleCaravan(pawns, faction, startingTile, addToWorldPawnsIfNotAlready);
      return false;
    }
    return true;
