...
internal class Patch_Combat : IPatchCategory
{
  ...
  private static bool TurretHitFlags(Thing thing, Projectile __instance, Thing ___launcher,
    ref bool __result)
  {
    if (__instance.TryGetComp<CompTurretProjectileProperties>() is not { } comp)
      return true;

    if (!thing.Spawned || thing == ___launcher)
    {
      __result = false;
      return false;
    }

    if (comp.hitflags == null || thing.def.fillPercent >= comp.hitflags.minFillPercent)
      return true;
    ...
    if (comp.hitflags.minFillPercent > 0)
    {
