...
namespace Vehicles
{
    public class Dialog_AssignSeats : Window
    {
        public Dialog_AssignSeats(List<TransferableOneWay> pawns, TransferableOneWay vehicle)
        {
            ...
            foreach(VehicleHandler handler in this.vehicle.GetCachedComp<CompVehicle>().handlers)
            {
        ...
        private void DrawVehicleMenu(Rect rect)
        {
            ...
            foreach(VehicleHandler handler in vehicle.GetCachedComp<CompVehicle>().handlers)
            {
                assignedRect.y += 30f;
                ...
                foreach(KeyValuePair<Pawn, Pair<VehiclePawn, VehicleHandler>> assignedKVP in assignedSeats.Where(r => r.Value.Second.role == handler.role))
                {
                    ...
                    if(!assignedKVP.Value.First.GetCachedComp<CompVehicle>().AllPawnsAboard.Contains(assignedKVP.Key) && Widgets.ButtonText(removalButtonRect, "RemoveFromRole".Translate()))
                    {
        ...
        private bool FinalizeSeats(out string failReason)
        {
            ...
            foreach(VehicleHandler handler in vehicle.GetCachedComp<CompVehicle>().handlers.Where(x => x.role.RequiredForCaravan))
            {
