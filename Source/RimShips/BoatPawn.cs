    ...
    public class BoatPawn : Pawn
    {
        ...
        public override IEnumerable<Gizmo> GetGizmos()
        {
            ...
            if(drafter != null)
            {
                ...
                foreach(Gizmo c2 in GetComp<CompShips>().CompGetGizmosExtra())
                {
                    yield return c2;
                }
                if(this.FueledBoat())
        ...
        public override void Kill(DamageInfo? dinfo, Hediff exactCulprit = null)
        {
            ...
            ThingDef shipDef = GetComp<CompShips>().Props.buildDef;
            ...
            if(flag)
            {
                if (map.terrainGrid.TerrainAt(position) == TerrainDefOf.WaterOceanDeep || map.terrainGrid.TerrainAt(position) == TerrainDefOf.WaterDeep)
                {
                    ...
                    string textPawnList = "";
                    foreach (Pawn p in GetComp<CompShips>()?.AllPawnsAboard)
                    {
                ...
                else
                {
                    ...
                    GetComp<CompShips>().DisembarkAll();
                    Destroy();
                }
            }
